<div align="center">
   ...
   <h1>ShopMood - Full-Stack MERN E-Commerce App</h1>
...
MONGO_URI=mongodb://127.0.0.1:27017/shopmood
...
From the **root folder** `shopmood/`, trigger a full install across environments:
```bash
npm run build
```
...
npm run seed
```
> **Seed Admin Access:** Email: `admin@shopmood.com` | Password: `password123`
...
This repository includes a fully-scaffolded API testing toolkit: **`ShopMood_Postman_Collection.json`**. 
...
- `POST /api/auth/refresh-token` - issue a new access token from a valid refresh token.
